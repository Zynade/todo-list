...
export default class UI {
  static addDefaultData() {
    ...
    const todaysDate = toDate(
      parse(todaysDateFormatted, 'dd-MM-yyyy', new Date()),
    );
  ...
  static addNewProject() {
    ...
    projectsTab.innerHTML += `
        ...
        `;
    document
      .querySelector('#new-project-add-button')
      .addEventListener('click', UI.addProjectHandler);
    document
      .querySelector('#new-project-cancel-button')
  ...
  static deleteProjectHandler() {
    ...
    UI.deleteProject();
    currentProjectName = Storage.getTodoList()
      .getProjects()
      .slice(-1)[0]
      .getProjectName();
    UI.initTasksPage();
  ...
  static addTaskItem(task) {
    ...
    const deleteButton = Array.from(
      document.querySelectorAll('.task-item-delete'),
    ).slice(-1)[0];
    const editButton = Array.from(
      document.querySelectorAll('.task-item-edit'),
    ).slice(-1)[0];
  ...
  static editTaskHandler(event) {
    if (forbiddenProjects.includes(currentProjectName)) {
      alert(
        'Sorry, currently you cannot edit tasks from this menu. Please visit the relevant Project tab to edit this task.',
      );
      return;
    }
    ...
    document
      .querySelector('#new-task-add-button')
    ...
    document
      .querySelector('#new-task-cancel-button')
      .addEventListener('click', UI.cancelTask);
  }

  static deleteTaskHandler(event) {
    if (forbiddenProjects.includes(currentProjectName)) {
      alert(
        'Sorry, currently you cannot delete tasks from this menu. Please visit the relevant Project tab to delete this task.',
      );
      return;
    }
  ...
  static generateTaskItems() {
    ...
    if (forbiddenProjects.includes(currentProjectName)) {
      ...
      Storage.getTodoList()
        ...
        .forEach((project) => {
          project.getTasks().forEach((task) => {
            ...
            if (
              isThisWeek(
                toDate(parse(task.getDate(), 'dd-MM-yyyy', new Date())),
              )
            ) {
      ...
      currentProject = currentProjectName === 'Today'
        ? today
        : currentProjectName === 'This Week'
          ? thisWeek
          : thisMonth;
      UI.hideNewTaskButton();
    } else {
    ...
    if (
      !document.querySelector('#new-task-button')
            && !forbiddenProjects.includes(currentProjectName)
    ) {
      UI.addNewTaskButton();
    }
  }
  ...
  static addNewTask() {
    ...
    document
      .querySelector('#new-task-add-button')
      .addEventListener('click', UI.addTaskHandler);
    document
      .querySelector('#new-task-cancel-button')
      .addEventListener('click', UI.cancelTask);
  }
